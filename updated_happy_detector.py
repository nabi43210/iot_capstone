...
print("model import success") #check model imported
score=0
total_score=0
video_score=0
total_video_score=0
...
for initial_tag in tags:
	...
	if check_viewed(name, movie).status_code == 200:
		movies.append(movie)
    initial.playlist.append(movie[0])

t2=threading.Thread(target=repeat, args=(model,))
t2.daemon=True
t2.start()

for index, movie in enumerate(initial_playlist):
    score=0
    total_score=0
    print("index: ",index)
    if(index==0):
        ...
        video_play(vid_list[index],movie)
        os.remove(vid_list[index])
        tag = recommand_algorithim.recommand_social(name)
	black_screen = pygame.Surface((800, 480))
	black_screen.fill((0, 0, 0))
...
while(1):
	...
	for movie in movies_origin:
        ...
        #태그당 동영상 2개씩 -> 총 6개 재생
        for index, movie in enumerate(play_list):
            score=0
            total_score=0
            if(index==0):
                vid=download_videos(movie,vid_list,str(index))
